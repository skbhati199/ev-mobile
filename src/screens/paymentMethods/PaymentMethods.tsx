...
export default class PaymentMethods extends BaseAutoRefreshScreen<Props, State> {
  ...
  public render = () => {
    ...
    const { paymentMethods, count, skip, limit, refreshing, loading, isAddModalVisible } = this.state;
    ...
    return (
      <Container style={style.container}>
        ...
        />
        <View>
          <TouchableOpacity onPress={() => this.setState({ isAddModalVisible: true })} style={style.addPMContainer}>
            <Icon style={style.addPMButton} type={'MaterialIcons'} name={'add'}></Icon>
          </TouchableOpacity>
          {isAddModalVisible && <CreatePaymentMethodDialog close={() => this.setState({ isAddModalVisible: false })} navigation={navigation} />}
        </View>
        {loading ? (
